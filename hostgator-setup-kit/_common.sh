...
# que trocar junto.
IMG_NS="ghcr.io/danilobsantos"
IMG_APP="${IMG_NS}/conecta-app"
IMG_WORKER="${IMG_NS}/conecta-worker"
IMG_SCHEDULER="${IMG_NS}/conecta-scheduler"
...
IMG_VOICE_AGENT="${IMG_NS}/conecta-voice-agent"
...
ultima_versao_publicada() {
  local url="${1:-https://github.com/danilobsantos/DeskcommCRM.git}" ref
...
trio_publicado() {
  local tag="$1" i
  for i in conecta-app conecta-worker conecta-scheduler conecta-voice-agent; do
