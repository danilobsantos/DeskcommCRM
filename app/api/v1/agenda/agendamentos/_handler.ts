...
export async function marcarAgendamentoHandler(
  ...
  // Com `provider_id`, o responsável é o profissional externo — a régua de
  // colegas (`owner_user_id`) não se aplica; a validação do profissional acima
  // já garantiu org e atividade.
  const ehDonoDoQueVaiNascer =
    ctx.actor.type === "user" && !donoProviderId && donoUserId === ctx.actor.id;
  if (!donoProviderId && aOpcaoPodeRecortar(ctx.actor, ehDonoDoQueVaiNascer)) {
