...
Mudou schema nesta dev → a tripla (migration + baseline + MANIFEST) vale igual, e o
executor Dokploy (`scripts/dokploy-migrate.sh`, inexistente no upstream) carimba por
TIMESTAMP: NNNN 9xxx com timestamp repetido faz uma migration pular a outra em
silêncio (medido no merge de 2026-09-11). Migrations daqui usam NNNN `9xxx`.
