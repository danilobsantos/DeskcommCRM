...
describe("renomear só o que a 0232 renomearia", () => {
  ...
  it("é a MESMA condição do backfill da 0232, lida do SQL que o clone aplica", () => {
    ...
    // Há DUAS linhas com `length(waha_session_name) > 54`: a do backfill da 9005
    // (renomeia tudo acima do teto, incondicional) e a da 0232 (só o que nunca
    // pareou e não está de pé — a MESMA condição do helper). É a da 0232 que
    // este caso vigia: pega a que carrega a condição.
    const backfill = baseline
      .split("\n")
      .find((l) => l.includes("length(waha_session_name) > 54") && l.includes("phone_number"));
