/**
 * O `imagens-ok` (check OBRIGATÓRIO) só aceita tudo `success`.
 *
 * O fork não pula build em PR (sem economia de alcance): o portão
 * (`a-tag-veio-da-production`) e os três resultados têm de ser `success`,
 * em qualquer evento. Toda porta que aceitasse `skipped` seria uma porta por
 * onde um desligamento passa verde (o buraco da issue #459, em
 * gatilho-dos-jobs-de-entrega.test.ts).
...
describe("imagens-ok só aceita o pulo declarado", () => {
  it("controle positivo: o recorte pegou o script que lê os quatro resultados", () => {
    for (const v of ["$PORTAO", "$BUILD", "$APP", "$FUNDO"]) {
  ...
  it("da matriz inteira de desfechos (512), passa exatamente o que foi declarado", { timeout: 60_000 }, () => {
    ...
    expect(passam.sort()).toEqual(
      [
        // Só tudo `success` passa — em qualquer evento, qualquer alcance.
        "pull_request sim success success success success",
        "pull_request nao success success success success",
