...
describe("scripts/sonda-pdf-na-imagem.mjs", () => {
  ...
  it("está no job que sobe a imagem do app, rodando dentro da imagem construída", () => {
    ...
    // A tag de fumaça leva o nome da imagem do fork (`conecta-smoke:pr`,
    // não `deskcomm-smoke:pr` do upstream) — ver `publish-image.yml`.
    expect(job.indexOf("conecta-smoke:pr")).toBeLessThan(job.indexOf("sonda-pdf-na-imagem.mjs"));
    // ...e a sonda roda DENTRO dela: mesma tag, executando o script montado.
    expect(job).toMatch(/conecta-smoke:pr node \/sonda-pdf-na-imagem\.mjs/);
