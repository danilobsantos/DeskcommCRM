...
const SEM_GRUPO: Record<string, string[]> = {
  ...
  "publish-image.yml": ["imagens-ok", "promover-stable", "a-tag-veio-da-production"],
};
    ...
    (arq, job) => {
      ...
      expect(cancela).toBe(
        arq === "publish-image.yml"
          // Fork: publica a partir da `production`, não da `main`.
          ? "${{ github.event_name == 'pull_request' || (github.event_name == 'push' && github.ref == 'refs/heads/production') }}"
