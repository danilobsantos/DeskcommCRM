...
export async function carregaRadarDeRisco(
  ...
  // Fase 1: leads e demandas em paralelo (independentes entre si). O `select`
  // das demandas traz `contacts(name, display_name)` porque o mapeamento lá
  // embaixo resolve o nome pelo módulo `nomeDoContato`.
  const [{ data: leads, error: leadsErr }, { data: semPasso, error: demandaErr }] = await Promise.all([
    consultaDeLeads.order("last_activity_at", { ascending: true, nullsFirst: true }).limit(SCAN_CAP),
    admin
      .from("demandas")
